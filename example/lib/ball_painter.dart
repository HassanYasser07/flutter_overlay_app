...
// **ابدأ الخط الأخضر من نفس نقطة نهاية الخط البني**
        double whiteBallNewAngle = _calculateWhiteBallPostCollisionAngle(
            cueAngle, whiteBallPosition, redBallPosition);

        List<Offset> whiteBallPath = _calculateReflectionPath(
            end, whiteBallNewAngle + (1.02 * pi), size, totalLineLength / 2);
...
// **حساب زاوية انعكاس الكرة الحمراء**
        double reflectedAngle = _calculateReflectedAngle(
            cueAngle, whiteBallPosition, redBallPosition);

// **حساب مسار الكرة الحمراء بعد التصادم (الخط الأصفر)**
...
// **رسم الخط الأصفر بعد التصادم**
        for (int j = 0; j < reflectedPath.length - 1; j++) {
          _drawDottedLine(
              canvas, reflectedPath[j], reflectedPath[j + 1], reflectedPaint);
        }
  ...
  double _calculateReflectedAngle(
    ...
    return 2 * normalAngle - incomingAngle + reflectionAngleAdjustment;
  }
...
//
//       Offset closestPoint = _closestPointOnSegment(redBallPosition, start, end);
//       Offset collisionPoint = redBallPosition + (closestPoint - redBallPosition).normalize() * ballRadius;
//
//       if ((closestPoint - redBallPosition).distance <= ballRadius) {
//         end = collisionPoint;
//         _drawDottedLine(canvas, start, end, dottedLinePaint);
//
//         double whiteBallNewAngle = _calculateWhiteBallPostCollisionAngle(
