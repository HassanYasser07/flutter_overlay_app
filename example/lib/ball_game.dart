...
class _BallGameState extends State<BallGameApp> {
  ...
  double initialCueAngle = 0.0; // تحديد نقطة جديدة لوضع عنصر التحكم بناءً على امتداد العصا
    ...
    return Center(
      child: FittedBox(
        child: Column(
          ...
          children: [
            IconButton(
              icon: Icon(showControls ? Icons.visibility_off : Icons.settings),
              onPressed:
            ...
            GestureDetector(
                ...
                setState(() {
                  if (initialTouchPosition != null) {
                    if (isCueRotating) {
                      ...
                      double oldAngle = atan2(initialCueTouchPosition!.dy - center.dy,
                    ...
                    if (isDraggingBall) { // ✅ فقط إذا كان المستخدم يسحب الكرة نفسها
                      if (isWhiteBallActive) {
                        // تحريك الكرة البيضاء فقط إذا كان السحب بدأ من عليها
                        final newWhitePosition = whiteBallPosition + details
                            .delta;
                        whiteBallPosition = Offset(
              ...
              onPanStart: (details) {
                setState(() {
                  ...
                  if ((dx - whiteBallPosition.dx).abs() < ballRadius &&
                    ...
                    isDraggingBall = true;  // ✅ السماح بالسحب فقط إذا كان المستخدم بدأ من الكرة
                  ...
                  //  isResizingTable = false;
                    initialCueTouchPosition = details.localPosition;  // ✅ تخزين نقطة اللمس الأولية
              ...
              child: Stack(
                children: [
                  ...
                  // وضع عناصر أخرى مثل الأزرار أو النوافذ
                  Positioned(
                    top: 0,
                    right: 0,
                    child: IconButton(
